        ...
        <div class="card-body">
            @if(adminAccessRoute(config('role.user_management.access.add')))
                <div class="media mb-4 float-right">
                    <a href="{{route('admin.casteCreate')}}" class="btn btn-sm btn-primary mr-2">
                    ...
                    @forelse($casteList as $item)
                        <tr>
                            ...
                            <td data-label="@lang('Religion Name')">
                                @lang(optional($item->religion)->name)
                            </td>

                            <td data-label="@lang('Action')">
                                @if(adminAccessRoute(config('role.user_management.access.edit')))
                                    <a href="{{ route('admin.casteEdit',$item->id) }}"
                                ...
                                @if(adminAccessRoute(config('role.user_management.access.delete')))
                                    <a href="javascript:void(0)"
                                       data-route="{{ route('admin.casteDelete', $item->id) }}"
                                       data-toggle="modal"
