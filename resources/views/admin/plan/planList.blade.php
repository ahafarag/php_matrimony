...
@section('content')
    ...
    <div class="card card-primary m-0 m-md-4 my-4 m-md-0 shadow">
        ...
        <div class="card-body">
            @if(adminAccessRoute(config('role.plan.access.add')))
                    ...
                    @forelse($managePlans as $item)
                        <tr>
                            ...
                            <td data-label="@lang('Name')">
                                @lang($item->details->name)
                            </td>
                            <td data-label="@lang('Plan Price')">
                                <p class="text-dark">{{ $basic->currency_symbol ?? '$' }}{{$item->price}}</p>
                            </td>
                            <td data-label="@lang('Status')">
                                <span class="badge badge-pill badge-{{ $item->status == 0 ? 'danger' : 'success' }}">
                                    {{ $item->status == 0 ? 'Deactive' : 'Active' }}
                                </span>
                            </td>
                            <td data-label="@lang('Action')">
                                @if(adminAccessRoute(config('role.plan.access.edit')))
                                    <a href="{{ route('admin.planEdit',$item->id) }}"
                                       class="btn btn-sm btn-primary edit-button">
                                        <i class="fa fa-edit" aria-hidden="true"></i>
                                    </a>
                                @endif

                                @if(adminAccessRoute(config('role.plan.access.delete')))
                                    <a href="javascript:void(0)"
                                       data-route="{{ route('admin.planDelete',$item->id) }}"
                                       data-toggle="modal"
